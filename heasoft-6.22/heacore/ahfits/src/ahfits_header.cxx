...
void writeKeyValStr(FilePtr ahffp, const std::string & key, 
  ...
  // keep existing comment string if input comment is an empty string
  std::string outcomment;
  ahfits::determineKeyComment(ahffp,key,comment,outcomment);
  char* tcomment=(char*)outcomment.c_str();
...
/// \brief Read the comment field of an existing keyword
/// \param[in] ahffp source FITS file object
/// \param[in] key header key label
/// \return header key comment
std::string getKeyComment(FilePtr ahffp, const std::string &key) {
  ...
  char value[FLEN_CARD];
  char comment[FLEN_CARD];
  if (0 != fits_read_key(ahffp->m_cfitsfp,TSTRING,key.c_str(),value,comment,
      &status)) {
    AH_THROW_RUNTIME(ahfits::errPrefix(ahffp,true,false)+"failed to read header keyword: "+key+
                     statusMsg(status));
  }
...
/// \brief (internal) Determine keyword comment string to write in 
///  writeKeyVal* functions.
/// \param[in] ahffp source FITS file object
/// \param[in] key header key label
/// \param[in] incomment key comment passed to writeKeyVal* function
/// \param[out] outcomment key comment to write to FITS file
///
/// This function is not intended to be used outside of the ahfits library.
/// It will determine the comment string to write to the FITS file in the
/// writeKeyVal* functions.  If the keyword is not present in the file,
/// outcomment=incomment.  If the keyword already exists in the file, then
/// outcomment=incomment if incomment is not an empty string.  If incomment
/// is an empty string, then outcomment will be set to the current keyword
/// comment in the file, so that writeKeyVal*() will not change the keyword
/// comment if incomment is an empty string.
void determineKeyComment(FilePtr ahffp, const std::string& key,
                         const std::string& incomment, std::string& outcomment) {

  if (!ahfits::keywordExists(ahffp,key)) {
    outcomment=incomment;
  } else {
    if (incomment == "") {
      outcomment=ahfits::getKeyComment(ahffp,key);
    } else {
      outcomment=incomment;
    }
  }

  // if outcomment contains only spaces, return an empty string (no comment)
  if (outcomment.find_first_not_of(' ') == std::string::npos) outcomment="";
}
