...
void writeImageCommon(FilePtr & ahffp, const std::string extname, double bzero,
  ...
  if(extname.empty()) {
    ...
    ahfits::writeKeyValStr(ahffp,"EXTNAME",extname,"name of this image extension");
    ahfits::move(ahffp,extname);     // Move to new extension
  }

  // write BZERO, BSCALE, and BNULL keywords
  ahfits::writeKeyValDbl(ahffp,"BZERO",bzero,"offset data range");  
  ahfits::writeKeyValDbl(ahffp,"BSCALE",bscale,"default scaling factor");
  if (bnull != 0) ahfits::writeKeyValLLong(ahffp,"BNULL",*bnull,"null value");
...
/// \brief Write images as type long long to FITS file
/// \param[in,out] ahffp  The FITS file object
/// \param[in] extname    Desired extension name to write FITS image to
/// \param[in] img        2D image vector to write to FITS image
/// \param[in] bzero      0th point if pixel values are not true physical values (Default 1.0)
...
/* Revision Log
   $Log$
   Revision 1.14  2017/02/13 18:49:06  mwitthoe
