...
void ut_read_header(void) {
  ...
  if (0 != ahffp) {
    ...
    START_TEST("read keyword comment") {
      std::string comm=ahfits::getKeyComment(ahffp,"TELESCOP");
      if (comm != "Mission name") FAIL;
    } END_TEST
...
void ut_write_header(void) {
  ...
  if (0 != ahffp) {
    ...
    START_TEST("overwrite keyword, string") {
      ...
      if (ahfits::getKeyComment(ahffp,"OBJECT") != "TESTCOMMENT") FAIL;
    } END_TEST
    ...
    START_TEST("write string; do not change comment") {
      ahfits::writeKeyValStr(ahffp,"TELESCOP","BLAH","");
      if (ahfits::getKeyValStr(ahffp,"TELESCOP") != "BLAH") FAIL;
      if (ahfits::getKeyComment(ahffp,"TELESCOP") != "Mission name") FAIL;
    } END_TEST
    ...
    START_TEST("write double") {
      ...
      if (ahfits::getKeyComment(ahffp,"DELAY1") != "TESTCOMMENT") FAIL;
    } END_TEST
    ...
    START_TEST("write long long") {
      ...
      if (ahfits::getKeyComment(ahffp,"CODE_ID") != "TESTCOMMENT") FAIL;
    } END_TEST
    ...
    START_TEST("write boolean") {
      ...
      if (ahfits::getKeyComment(ahffp,"CLOCKAPP") != "TESTCOMMENT") FAIL;
    } END_TEST
    ...
    // test internal function used for updating keyword comments
    START_TEST("test determineKeyComment") {
      std::string outcomment="";
      ahfits::determineKeyComment(ahffp,"TELESCOP","new comment",outcomment);
      if (outcomment != "new comment") FAIL;
      ahfits::determineKeyComment(ahffp,"TELESCOP","",outcomment);
      if (outcomment != "Mission name") FAIL;
      ahfits::determineKeyComment(ahffp,"TELESCOP","    ",outcomment);
      if (outcomment != "") FAIL;
    } END_TEST
  }
...
/* Revision Log
 $Log$
 Revision 1.17  2017/02/13 18:49:06  mwitthoe
