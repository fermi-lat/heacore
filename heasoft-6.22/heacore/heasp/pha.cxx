			     ...
			     const Integer EndChannel)
{
  ...
  // first need to convert Pha data to counts
  vector<Integer> dataCounts(Nchan);
  if ( AreaScaling.size() > 0 ) {
    for (size_t i=0; i<Nchan; i++) dataCounts[i] = Pha[i] * Exposure * AreaScaling[i];
  } else {
    for (size_t i=0; i<Nchan; i++) dataCounts[i] = Pha[i] * Exposure;
  }
			       ...
			       const Integer EndChannel, const pha& Background)
{
  ...
  if ( Poisserr ) {
    if ( AreaScaling.size() == 0 ) {
      for (size_t i=0; i<Nchan; i++) Noise[i] = sqrt(Pha[i]*Exposure);
    } else {
      for (size_t i=0; i<Nchan; i++) Noise[i] = sqrt(Pha[i]*Exposure*AreaScaling[i]);
    }
  }
  ...
  if ( Background.Pha.size() != 0 ) {
    ...
    for (size_t i=0; i<Nchan; i++) {
      Scale[i] = Exposure*AreaScaling[i]*BackScaling[i]
	/(Background.Exposure*Background.AreaScaling[i]*Background.BackScaling[i]);
    }
    ...
    for (size_t i=0; i<Nchan; i++) {
	  ...
	  + Background.StatError[i]*Scale[i]*Background.StatError[i]*Scale[i]);
    }

  }

  vector<Real> SignalDivideNoise(Signal.size());
  for (size_t i=0; i<Nchan; i++) SignalDivideNoise[i] = Signal[i]/Noise[i];

  // convert start and end channel to zero-based
  Integer start = StartChannel - FirstChannel;
  Integer end = EndChannel - FirstChannel;
  grouping groupInfo;
  groupInfo.loadMin(SignalToNoise, start, end, SignalDivideNoise);
